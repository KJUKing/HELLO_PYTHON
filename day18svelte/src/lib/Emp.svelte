<script>
    ...
    let vo = {'e_id': '_', 'e_name': '_', 'e_gen': '_', 'e_addr': '_'};
    ...
    let fn_del = () => {
        ...
        let param = vo
        fetch('http://127.0.0.1/emp.del.ajax', {
            method: "POST",
            headers: {"Content-Type": "application/json"},
                body: JSON.stringify(param)
            })
...
<table>
    ...
    <!--{#each mylist as {e_id, e_name, e_gen, e_addr}}-->
    <!--    <tr>-->
    <!--        <td>{e_id}</td>-->
    <!--        <td>{e_name}</td>-->
    <!--        <td>{e_gen}</td>-->
...
<table>
    ...
    <tr>
        ...
        <td>성별</td>
        <td>
            <input type="text" bind:value="{vo.e_gen}"/>
        </td>
    </tr>
    <tr>
        ...
        <td>주소</td>
        <td>
            <input type="text" bind:value="{vo.e_addr}"/>
        </td>
    </tr>
    <tr>
